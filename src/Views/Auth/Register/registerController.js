...
seccional.addEventListener('change',async () => {
    adjuntarOpc.adjuntarReseteo(organizacion,`organizations/sectional/${seccional.value}`); }
);
window.addEventListener("click", async (e) => {
    if (e.target.matches("#tengoCuenta") && !window.procesoPeticion) window.location.href = '#/login';
});
}